...
class DeeplabV3Plus(tf.keras.Model):
    ...
    def _get_backbone_feature(self, input_shape, feature_name) -> tf.keras.Model:
        ...
        backbone = Xception(input_tensor=input_layer, weights='imagenet', include_top=False)
        output_layer = backbone.get_layer(feature_name).output
        return tf.keras.Model(inputs=input_layer, outputs=output_layer)

    def build(self, input_shape):
        self.backbone_feature_1 = self._get_backbone_feature(input_shape, 'block13_sepconv2_bn')
        self.backbone_feature_2 = self._get_backbone_feature(input_shape, 'block4_sepconv2_bn')
        ...
        self.out_conv = tf.keras.layers.Conv2D(self.num_classes, kernel_size=(1, 1), padding='same', dtype='float32')
    ...
    def call(self, inputs, training=False, mask=None):
        ...
        input_b = tf.keras.layers.UpSampling2D(size=tf.shape(input_a)[1:3], interpolation='bilinear')(input_b)
